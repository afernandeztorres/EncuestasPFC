...
public class ManagerBiEncuestas implements MyManagerBiDefault{

    MyDaoDefault daoImpl = new EncuestasDaoImpl();
    ...
    public Encuesta find(Encuesta encuesta) throws Exception{
        ...
        if ( encuesta != null ){
            ...
            for (int i = 0 ; i < encuesta.getPreguntas().size() ; i ++){
                daoImpl.connect();
                encuesta.getPreguntas().get(i).setRespuestas(daoImpl.findRepuestas(
                                encuesta.getPreguntas().get(i).getId_Pregunta(),encuesta.getId_Tbl_Idioma() ));
            }
        }
        return encuesta;
    }
    ...
    public MyDaoDefault getDaoImpl() {
        return daoImpl;
    }
